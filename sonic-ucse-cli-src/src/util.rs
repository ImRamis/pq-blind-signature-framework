...
use pairing::{
    ...
    CurveAffine, CurveProjective, Engine, Field, PrimeField, PrimeFieldRepr,
};
use sapling_crypto::jubjub::{edwards::Point, JubjubEngine, PrimeOrder};
use std::io::{self, Read, Write};

use dusk_jubjub::JubJubExtended;
use curv::arithmetic::Converter; // Import the necessary trait
...
/// Converts a big-endian vector of booleans to a BigInt.
pub fn bool_vec_to_big_int(vec: &Vec<Option<bool>>) -> curv::BigInt {
    let mut s = String::with_capacity(vec.len());
    for b_opt in vec {
        s.push(if b_opt.unwrap_or(false) { '1' } else { '0' });
    }
    curv::BigInt::from_str_radix(&s, 2).unwrap_or_else(|_| curv::BigInt::from(0))
}

/// Converts a big-endian vector of booleans to a vector of JubJub scalars.
/// The vector is chunked into 48-bit segments as required by the circuit.
pub fn be_opt_vec_to_jubjub_scalar(vec: &Vec<Option<bool>>) -> Vec<JubJubScalar> {
    let mut scalars = vec![];
    for chunk in vec.chunks(48) {
        let bigint = bool_vec_to_big_int(&chunk.to_vec());
        // .to_bytes() returns big-endian, but from_bytes_wide expects little-endian.
        let mut be_bytes = bigint.to_bytes();
        be_bytes.reverse(); // Reverse to convert to little-endian.

        // from_bytes_wide expects a 64-byte little-endian array.
        let mut wide_bytes = [0u8; 64];
        let bytes_to_copy = std::cmp::min(be_bytes.len(), 64);
        wide_bytes[..bytes_to_copy].copy_from_slice(&be_bytes[..bytes_to_copy]);

        let scalar = JubJubScalar::from_bytes_wide(&wide_bytes);
        scalars.push(scalar);
    }
    scalars
}
...
pub fn opt_vec_to_bytes(vec: &Vec<Option<bool>>) -> Vec<u8> {
    let out: Vec<bool> = vec.iter().map(|x| x.unwrap_or(false)).collect::<Vec<bool>>();
...
pub fn to_be_bytes<E: Engine>(
    ...
    let mut bytes = [&pi.to_bytes(), x_bytes].concat();
    let mut c_bytes = cts.iter().flat_map(|c| c.to_bytes()).collect::<Vec<_>>();
    bytes.append(&mut c_bytes);
    bytes.append(
        &mut [
            pk_l.to_bytes().as_slice(),
            sigma.to_bytes().as_slice(),
        ]
        .concat(),
    );
    bytes
...
// --- Corrected Serialization Helpers ---

pub fn write_point<W: Write, E: JubjubEngine>(
    writer: &mut W,
    p: &Point<E, PrimeOrder>,
) -> Result<()> {
    p.write(writer)
        .map_err(|e| anyhow!("Failed to write point: {}", e))
}

pub fn read_point<R: Read, E: JubjubEngine>(
    reader: &mut R,
    params: &E::Params,
) -> Result<Point<E, PrimeOrder>> {
    let point_unknown =
        Point::read(reader, params).map_err(|e| anyhow!("Failed to read point: {}", e))?;
    // The point is read into an 'Unknown' subgroup, so we must convert it
    // to PrimeOrder for use in the circuit.
    point_unknown
        .as_prime_order(params)
        .ok_or_else(|| anyhow!("Point is not in the prime order subgroup"))
}
    ...
    U: Iterator<Item = T::Item>,
{
    ...
    fn next(&mut self) -> Option<T::Item> {
        self.t.next().or_else(|| self.u.next())
    }
}
    ...
    U: DoubleEndedIterator,
{
    fn next_back(&mut self) -> Option<T::Item> {
        self.u.next_back().or_else(|| self.t.next_back())
    }
}
