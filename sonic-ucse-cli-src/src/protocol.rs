...
use pairing::{
    CurveAffine, CurveProjective, EncodedPoint, Engine, Field, PrimeField, PrimeFieldRepr,
};
...
impl<E: Engine> SonicProof<E> {
    ...
    pub fn write<W: Write>(&self, writer: &mut W) -> Result<()> {
        writer.write_all(self.r.into_uncompressed().as_ref())?;
        writer.write_all(self.t.into_uncompressed().as_ref())?;
        writer.write_all(self.z_opening.into_uncompressed().as_ref())?;
        writer.write_all(self.zy_opening.into_uncompressed().as_ref())?;

        // The representation of a field element is 32 bytes (256 bits).
        const FR_SIZE: usize = 32;
        let mut fr_bytes = vec![0u8; FR_SIZE];

        self.rz.into_repr().write_le(&mut fr_bytes[..])?;
        writer.write_all(&fr_bytes)?;

        self.rzy.into_repr().write_le(&mut fr_bytes[..])?;
        writer.write_all(&fr_bytes)?;
        Ok(())
    }
    ...
    pub fn read<R: Read>(reader: &mut R) -> Result<Self> {
        ...
        let r = g1_repr
            .into_affine()
            .map_err(|e| anyhow!("G1 decompression failed for r: {}", e))?;

        reader.read_exact(g1_repr.as_mut())?;
        let t = g1_repr
            .into_affine()
            .map_err(|e| anyhow!("G1 decompression failed for t: {}", e))?;

        reader.read_exact(g1_repr.as_mut())?;
        let z_opening = g1_repr
            .into_affine()
            .map_err(|e| anyhow!("G1 decompression failed for z_opening: {}", e))?;

        reader.read_exact(g1_repr.as_mut())?;
        let zy_opening = g1_repr
            .into_affine()
            .map_err(|e| anyhow!("G1 decompression failed for zy_opening: {}", e))?;
        ...
        reader.read_exact(&mut fr_bytes)?;
        fr_repr.read_le(&fr_bytes[..])?;
        let rz =
            E::Fr::from_repr(fr_repr).map_err(|e| anyhow!("Fr decoding failed for rz: {}", e))?;

        reader.read_exact(&mut fr_bytes)?;
        fr_repr.read_le(&fr_bytes[..])?;
        let rzy =
            E::Fr::from_repr(fr_repr).map_err(|e| anyhow!("Fr decoding failed for rzy: {}", e))?;
...
pub struct UCProof<E: Engine> {
    pub c: Vec<jubjub_elgamal::Cypher>,
    pub pi: SonicProof<E>,
    pub pk_l: PublicKey,
    pub sigma: Signature,
    pub pk_ot: schnorrots::PublicKey,
    pub sigma_ot: schnorrots::Signature,
}

impl<E: Engine> UCProof<E> {
    pub fn write<W: Write>(&self, writer: &mut W) -> Result<()> {
        writer.write_all(&(self.c.len() as u64).to_le_bytes())?;
        for ctext in &self.c {
            writer.write_all(&ctext.to_bytes())?;
        }

        self.pi.write(&mut *writer)?;

        writer.write_all(&self.pk_l.to_bytes())?;
        writer.write_all(&self.sigma.to_bytes())?;
        writer.write_all(&self.pk_ot.to_bytes())?;
        writer.write_all(&self.sigma_ot.to_bytes())?;
    ...
    pub fn read<R: Read>(reader: &mut R) -> Result<Self> {
        ...
        let mut c = Vec::with_capacity(len as usize);
        for _ in 0..len {
            let mut c_bytes = [0u8; 64];
            reader.read_exact(&mut c_bytes)?;
            let ctext = jubjub_elgamal::Cypher::from_bytes(c_bytes)
                .map_err(|e| anyhow!("Failed to deserialize ElGamal ciphertext: {:?}", e))?;
            c.push(ctext);
        }

        let pi = SonicProof::<E>::read(&mut *reader)?;

        let mut pk_l_bytes = [0u8; 32];
        reader.read_exact(&mut pk_l_bytes)?;
        let pk_l = PublicKey::from_bytes(&pk_l_bytes)
            .map_err(|e| anyhow!("Failed to read PublicKey: {:?}", e))?;

        let mut sigma_bytes = [0u8; 64];
        reader.read_exact(&mut sigma_bytes)?;
        let sigma = Signature::from_bytes(&sigma_bytes)
            .map_err(|e| anyhow!("Failed to deserialize Signature: {:?}", e))?;

        let mut pk_ot_bytes = [0u8; 32];
        reader.read_exact(&mut pk_ot_bytes)?;
        let pk_ot = schnorrots::PublicKey::from_bytes(&pk_ot_bytes)
            .map_err(|e| anyhow!("Failed to deserialize OTS PublicKey: {:?}", e))?;

        let mut sigma_ot_bytes = [0u8; 64];
        reader.read_exact(&mut sigma_ot_bytes)?;
        let sigma_ot = schnorrots::Signature::from_bytes(&sigma_ot_bytes)
            .map_err(|e| anyhow!("Failed to deserialize OTS Signature: {:?}", e))?;
...
impl<E: Engine> Aggregate<E> {
    pub fn write<W: Write>(&self, writer: &mut W) -> Result<()> {
        ...
        writer.write_all(self.s_opening.into_uncompressed().as_ref())?;
        writer.write_all(self.opening.into_uncompressed().as_ref())?;

        // Write the vector of openings
        writer.write_all(&(self.c_openings.len() as u64).to_le_bytes())?;
        for (point, scalar) in &self.c_openings {
            writer.write_all(point.into_uncompressed().as_ref())?;
            let mut scalar_bytes = vec![];
            scalar.into_repr().write_le(&mut scalar_bytes)?;
            writer.write_all(&scalar_bytes)?;
        }
        Ok(())
    }
    ...
    pub fn read<R: Read>(reader: &mut R) -> Result<Self> {
        ...
        reader.read_exact(g1_repr.as_mut())?;
        let c = g1_repr.into_affine().map_err(|e| anyhow!("G1 decompression failed for c: {}", e))?;
        
        reader.read_exact(g1_repr.as_mut())?;
        let s_opening = g1_repr.into_affine().map_err(|e| anyhow!("G1 decompression failed for s_opening: {}", e))?;
        
        reader.read_exact(g1_repr.as_mut())?;
        let opening = g1_repr.into_affine().map_err(|e| anyhow!("G1 decompression failed for opening: {}", e))?;

        // Read the vector of openings
        let mut len_bytes = [0u8; 8];
        reader.read_exact(&mut len_bytes)?;
        let len = u64::from_le_bytes(len_bytes) as usize;

        let mut c_openings = Vec::with_capacity(len);
        const FR_SIZE: usize = 32;
        let mut fr_bytes = vec![0u8; FR_SIZE];

        for _ in 0..len {
            reader.read_exact(g1_repr.as_mut())?;
            let point = g1_repr.into_affine().map_err(|e| anyhow!("G1 decompression failed for c_opening point: {}", e))?;
            
            reader.read_exact(&mut fr_bytes)?;
            let mut fr_repr = E::Fr::zero().into_repr();
            fr_repr.read_le(&fr_bytes[..])?;
            let scalar = E::Fr::from_repr(fr_repr).map_err(|e| anyhow!("Fr decoding failed for c_opening scalar: {}", e))?;

            c_openings.push((point, scalar));
        }

        Ok(Aggregate { c, s_opening, c_openings, opening })
    }
}
